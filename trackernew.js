...
const lookup = async () => {
  const fromBlock = document.querySelector("input[name=fromBlock]").value;
  ...
  try {
    ...
    summaryTable.innerHTML = `
      ...
      ).toFixed(2)} USD (${Number(unlockedSnx).toFixed(2)} SNX) ${Math.round(
  ...
  // show debt balance over time chart
  const currentBlockNumber = await snxjs.ethers
    .getDefaultProvider()
    .getBlockNumber();
  const blocks = [];
  let debtBalances = [];
  ...
  for (let i = 0; i < 10; i++) {
    blocks.push(currentBlockNumber - i * 10000); // about a day ago
  }
  ...
  ).map(snxjs.utils.formatEther);

  const chart2 = new frappe.Chart(chartTrendingDebtContainer, {
    title: "Debt balance over time",
    data: {
      labels: blocks.reverse(),
      datasets: [
        {
          name: "Debt Balance",
          type: "line",
          values: debtBalances.reverse()
        }
      ]
    },
    type: "line",
